...
import { Donor, DonorSchema } from "@repo/zod-utils";
...
import { updateDonor } from "@/app/actions/donors";
import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";
import { CircleX } from "lucide-react";
import { useToast } from "@/hooks/use-toast";

export const EditDonorForm: React.FC<{ donorData: Donor }> = ({
  donorData,
}) => {
  ...
  const form = useForm<Donor>({
    ...
    defaultValues: {
      ...
      cnicBack: donorData.cnicBack || "",
      cnicFront: donorData.cnicFront || "",
      profilePic: donorData.profilePic || "",
  ...
  useEffect(() => {
    form.setValue("name", donorData.name || "");
    form.setValue("fatherName", donorData.fatherName || "");
    form.setValue("cnic", donorData.cnic);
    form.setValue("phone", donorData.phone);
    form.setValue("cnicBack", donorData.cnicBack);
    form.setValue("cnicFront", donorData.cnicFront);
    form.setValue("profilePic", donorData.profilePic);
    setProfilePicItem(donorData.profilePic || "");
    setCnicBackItem(donorData.cnicBack || "");
    setCnicFrontItem(donorData.cnicFront || "");
    form.setValue("address", donorData.address);
    form.setValue("city", donorData.city);
    console.log("Form values: ", donorData);
  }, [donorData]);
  ...
  async function onSubmit(values: Donor) {
    const result = await updateDonor(donorData.id || "", values);
    if (!result.success) {
      toast.toast({
        variant: "destructive",
        title: "Failed to create",
        description: `Donor cannot be updated at the moment. ${result.data}`,
      });
    } else {
      toast.toast({
        title: "Updated",
        description: "Donor has been updated successfully.",
      });
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      // {...field}
                    />
                    {/* <Image src={donorData.profilePic} alt={donorData.name} width={200} height={200} className="aspect-square" /> */}
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
