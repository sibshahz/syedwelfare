...
const PaymentDialog:React.FC<{id: string, name: string, cnic: string}>=({id, name, cnic}: {id: string, name: string, cnic: string})=>{
  ...
  <AlertDialogContent>
    <AlertDialogHeader>
      <AlertDialogTitle>Please enter the amount recieved from the donor.</AlertDialogTitle>
      <AlertDialogDescription>
    ...
    <AlertDialogFooter>
      ...
      <AlertDialogAction onClick={async() => payDonorAmount(id)}
        disabled={amount === 0}
        className='bg-green-500 text-white hover:bg-green-600'
        >Add donation</AlertDialogAction>
    </AlertDialogFooter>
  </AlertDialogContent>
</AlertDialog>
  )
...
const DeleteDialog:React.FC<{id: string}>=({id}: {id: string})=>{
  ...
  <AlertDialogContent>
    <AlertDialogHeader>
      ...
      <AlertDialogDescription>
        This action cannot be undone. This will permanently delete donor data.
      </AlertDialogDescription>
    </AlertDialogHeader>
    <AlertDialogFooter>
      <AlertDialogCancel>Cancel</AlertDialogCancel>
...
const DonorsTableItem: React.FC<DonorsTableItemProps> = ({ donor }) => {
  const { id, cnic, name, fatherName, phone, address, city, profilePic} = donor;
